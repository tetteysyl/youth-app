...
import { sendMeetingRoleEmail } from "@/lib/email";
import { format } from "date-fns";

/**
 * GET /api/cron/meeting-reminders — runs daily via Vercel Cron.
 *
 * Reminds the MC and the Meeting Leader of their duty:
 *   • 3 days before the meeting
 *   • 1 day (24 hours) before the meeting
 *
 * Each reminder sends an in-app notification AND an email. Meetings carry
 * `remind3DaySent` / `remind1DaySent` flags so a reminder is never sent twice,
 * even if the cron runs more than once in a day.
 *
  ...
  try {
    ...
    for (const w of windows) {
      ...
      for (const doc of snap.docs) {
        ...
        // Nothing to remind — still flag it so we don't re-scan every run.
        if (duties.length === 0) {
          ...
          if (member.email) {
            try {
              await sendMeetingRoleEmail(
                ...
                d.duty,
                { title: m.title ?? "", date: prettyDate, time: m.time ?? "", location: m.location, agenda: m.agenda },
                w.label
              );
              emailed++;
            } catch (e) {
