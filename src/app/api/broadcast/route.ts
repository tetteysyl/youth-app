...
// GET /api/broadcast — list broadcast history
...
// POST /api/broadcast — save broadcast + notify all members + email all members
export async function POST(req: NextRequest) {
  try {
    const { subject, message, senderId, senderName } = await req.json();
    if (!subject || !message) {
    ...
    // Bell notification for every active member
    ...
    // Email (best-effort — does not fail the broadcast if email sending breaks)
    try {
